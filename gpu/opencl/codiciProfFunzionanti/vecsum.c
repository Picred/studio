...
cl_event vecsum(cl_command_queue que, cl_kernel vecsum_k,
	...
	cl_int nels, cl_mem d_v1, cl_mem d_v2, cl_mem d_v3,
	cl_uint num_events_in_wait_list, const cl_event *event_wait_list)
{
	...
	ocl_check(err, "set arg %d to vecsum_k", arg);
	++arg;
	err = clSetKernelArg(vecsum_k, arg, sizeof(nels), &nels);
	ocl_check(err, "set arg %d to vecsum_k", arg);
...
int main(int argc, char* argv[])
{
	...
	cl_kernel vecsum_k = clCreateKernel(prog, "vecsum_v1", &err);
	...
	cl_event vecsum_evt = vecsum(que, vecsum_k, vecsum_lws_multiple,
		cli_lws, nels, d_v1, d_v2, d_v3, 1, &vecinit_evt);
