...
cl_event vecinit(cl_command_queue que, cl_kernel vecinit_k, int nels, cl_mem d_array){
    ...
    err = clEnqueueNDRangeKernel(que, vecinit_k, 1, NULL, gws, NULL, 0, NULL, &vecinit_k);
...
int main(int argc, char*argv[]){	
    ...
    cl_program prog = create_program("vecinit.ocl", ctx, d); //codice device + contesto + device d
